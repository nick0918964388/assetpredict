export default {
  ...
  getProductWarehouses: async () => {
    const productWarehousesRaw = await getProductLocations.run({
      variationId: tbl_lowhealth.triggeredRow.VariationId,
    });
  ...
  updateProduct: async (updateType) => {
    ...
    try {
      if (updateType === 'TABLE') {
        updateProductParams = {
          productId: tbl_lowhealth.updatedRow.Id,
          name: tbl_lowhealth.updatedRow.Name,
          description: tbl_lowhealth.updatedRow.Description,
          type: tbl_lowhealth.updatedRow.Category,
					image: tbl_lowhealth.updatedRow.Image,
        };
        updateVariantParams = {
          variantId: tbl_lowhealth.updatedRow.VariationId,
          costPrice: tbl_lowhealth.updatedRow.CostPrice,
          salePrice: tbl_lowhealth.updatedRow.SalePrice,
          lowStock: tbl_lowhealth.updatedRow.LowStock || 0,
          sku: tbl_lowhealth.updatedRow.Sku || null,
        };

      } else {
        updateProductParams = {
          productId: tbl_lowhealth.triggeredRow.Id,
        ...
        updateVariantParams = {
          variantId: tbl_lowhealth.triggeredRow.VariationId,
